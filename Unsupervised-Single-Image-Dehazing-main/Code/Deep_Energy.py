...
from train_loss import segmentation_Loss
...
for i in range(num_iter):
    ...
    if (args.print_iter > 0) and (i % args.print_iter == 0):
        _, loss_value, loss_smooth, loss_data, net_out = sess.run([train_op, loss, \
                                                                   app_loss.loss_smooth, app_loss.loss_data, output_from_network],
                                                                  feed_dict=feed_dict)
    ...
    if (i % (samples_in_epoch) == 0) & (i > 0):
        ...
        if args.app == 'seg':
            ...
            print("epoch: %d, avg_test_loss: %.3f, test_MSE_t1: %.3f, test_MSE_t2: %.3f, lr: %.5f" % (
            epoch, test_loss, test_MSE_t1, test_MSE_t2, curr_lr))
